...
export default function Header(props) {
    ...
    return (
        <section className="PrimaryHeader lg:h-20 z-50 px-3 flex items-center justify-between relative">
            <img src={props.imgURL} alt="Logo" className="logo lg:w-32" />

            <div className="menu lg:flex hidden lg:py-5 lg:font-semibold justify-evenly w-2/5 relative">
    {menuLinks.map((link) => (
        <div
        ...
        >
            <a
                className="hover:text-red-600 lg:px-4 transition-all duration-300 lg:py-3"
                href={link.href}
            >
                {link.name}
            </a>
            ...
            <div className="flex items-center gap-5 ">
                ...
                >
                    <div className={`h-1 bg-black rounded transition-all ${isMenuOpen ? 'rotate-45 translate-y-2' : ''}`} />
                    <div className={`h-1 bg-black rounded transition-all ${isMenuOpen ? 'opacity-0' : ''}`} />
                    <div className={`h-1 bg-black rounded transition-all ${isMenuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
                </div>
            </div>
            ...
            {/* Mobile Menu */}
            {isMenuOpen && (
                <div className="toggleBar  absolute top-16 right-3 w-96 bg-white shadow-lg rounded-lg p-4 flex flex-col gap-3 md:hidden">
                    {menuLinks.map(link => (
                        <a key={link.name} href={link.href} className="text-black font-semibold">{link.name}</a>
                    ))}
                </div>
            )}
        </section>
